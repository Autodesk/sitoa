...
AI_SHADER_NODE_EXPORT_METHODS(SIBScalarPassThroughMethods);

enum SIBScalarPassThroughParams
{
   p_input,
   p_channel1,
   p_channel2,
...
node_parameters
{
   ...
   AiParameterFlt( "channel1", 0.0f);
   AiParameterFlt( "channel2", 0.0f);
   AiParameterFlt( "channel3", 0.0f);
   AiParameterFlt( "channel4", 0.0f);
   AiParameterFlt( "channel5", 0.0f);
   AiParameterFlt( "channel6", 0.0f);
   AiParameterFlt( "channel7", 0.0f);
   AiParameterFlt( "channel8", 0.0f);
}
...
shader_evaluate
{
  bool* activeChannel = (bool*)AiNodeGetLocalData(node);
   ...
   for (int i = 0; i < 8; i++)
      if (activeChannel[i])
         AiShaderEvalParamFlt(p_channel1 + i);   
