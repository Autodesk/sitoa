...
AI_SHADER_NODE_EXPORT_METHODS(MapLookupIntegerMethods);

enum MapLookupIntegerParams 
{
   p_map,
   p_factor,
};
...
shader_evaluate
{
   ...
   if (!paramEntry)
   {
      ...
      if (clipData && clipData->m_isValid)
      {
         ...
         sg->out.INT() = (int)((color.r + color.g + color.b) * .333f * factor);
      }
   }
   else
   {
      ...
      if (paramType == AI_TYPE_FLOAT)
      {
         ...
         // sg->out.INT() = (int)ROUND(val * factor);
         // AiUDataGetFlt always interpolates, also if the 3 values of the tria are the same
         // For instance, if a weightmap is all 1s, the returned value is very often something
         // like 0.9999, so by casting to int you get 0. mental ray does the same, let's keep
         // the standard behavior
         sg->out.INT() = (int)(val * factor);
      }
