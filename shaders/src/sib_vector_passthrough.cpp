...
AI_SHADER_NODE_EXPORT_METHODS(SIBVectorPassThroughMethods);

enum SIBVectorPassThroughParams
{
   p_input,
   p_channel1,
   p_channel2,
...
node_parameters
{
   ...
   AiParameterVec( "channel1", 0.0f, 0.0f, 0.0f );
   AiParameterVec( "channel2", 0.0f, 0.0f, 0.0f );
   AiParameterVec( "channel3", 0.0f, 0.0f, 0.0f );
   AiParameterVec( "channel4", 0.0f, 0.0f, 0.0f );
   AiParameterVec( "channel5", 0.0f, 0.0f, 0.0f );
   AiParameterVec( "channel6", 0.0f, 0.0f, 0.0f );
   AiParameterVec( "channel7", 0.0f, 0.0f, 0.0f );
   AiParameterVec( "channel8", 0.0f, 0.0f, 0.0f );
}
...
shader_evaluate
{
   ...
   for (int i = 0; i < 8; i++)
      if (activeChannel[i])
         AiShaderEvalParamVec(p_channel1 + i);
