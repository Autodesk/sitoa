...
AI_SHADER_NODE_EXPORT_METHODS(SIBAttributeVectorMethods);

enum SIBAttributeVectorParams 
{
   p_attribute,
   p_index,
   p_default
...
node_parameters
{
   AiParameterStr ("attribute"  , "Vector");
...
shader_evaluate
{
   ...
   if (data->attribute && AiUDataGetVec(data->attribute, sg->out.VEC()))
      return;
