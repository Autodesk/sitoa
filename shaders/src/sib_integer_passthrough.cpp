...
AI_SHADER_NODE_EXPORT_METHODS(SIBIntegerPassThroughMethods);

enum SIBIntegerPassThroughParams
{
   p_input,
   p_channel1,
   p_channel2,
...
node_parameters
{
   ...
   AiParameterInt( "channel1", 0);
   AiParameterInt( "channel2", 0);
   AiParameterInt( "channel3", 0);
   AiParameterInt( "channel4", 0);
   AiParameterInt( "channel5", 0);
   AiParameterInt( "channel6", 0);
   AiParameterInt( "channel7", 0);
   AiParameterInt( "channel8", 0);
}
...
shader_evaluate
{
   ...
   for (int i = 0; i < 8; i++)
      if (activeChannel[i])
         AiShaderEvalParamInt(p_channel1 + i);

   sg->out.INT() = AiShaderEvalParamInt(p_input);
}
