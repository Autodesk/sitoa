...
#define COUNT_CALLS 0

AI_SHADER_NODE_EXPORT_METHODS(txt2d_image_explicitMethods);

enum txt2d_image_explicitParams 
{
   p_tex,
   p_tspace_id,
   p_repeats,
...
node_parameters
{
   ...
   AiParameterStr ( "tspace_id"    , "" );               // NOT USED ANYMORE AFTER FIXING #505
   ...
   AiParameterBool( "alpha_output" , false );                     // Not Implemented 
   AiParameterFlt ( "alpha_factor" , 1.0f );                      // Not Implemented 
...
#if COUNT_CALLS
   AtUInt64 call_count[AI_MAX_THREADS];
#endif
} ShaderData;
...
#if COUNT_CALLS
   ShaderData *data = (ShaderData*)AiNodeGetLocalData(node);
   AtUInt64 total_count = 0;
   for (int i = 0; i < AI_MAX_THREADS; i++)
      total_count+= data->call_count[i];

   const char* name = AiNodeGetName(node);
   AiMsgInfo("------>>>>>> Number of calls for %s = %u", name, total_count);
#endif
...
shader_evaluate
{
   ...
   // For #1325, we must let uv > 1 go, since they could be needed in case of <udim> filenames by sib_image_clip

   // Backup original uv and derivatives
   BACKUP_SG_UVS;
   ...
   // Evaluate texture
   AtRGBA tex_source = AiShaderEvalParamRGBA(p_tex);
   if (data->alpha_output)
      tex_source.r = tex_source.g = tex_source.b = tex_source.a;

   sg->out.RGBA() = tex_source;
  
   // Restore original UV's and derivatives
   RESTORE_SG_UVS;
}
