...
AI_SHADER_NODE_EXPORT_METHODS(SIBAttributeIntegerMethods);

enum SIBAttributeIntegerParams 
{
   p_attribute,
   p_index,
   p_default
...
node_parameters
{
   AiParameterStr ("attribute"  , "Integer");
...
shader_evaluate
{
   ...
   if (!data->attribute.empty() && AiUDataGetInt(data->attribute, sg->out.INT()))
      return;
